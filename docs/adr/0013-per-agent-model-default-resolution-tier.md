...
**Effort levels are model-specific.** pi-ai declares the valid levels per model; a model supports the subset of `off | minimal | low | medium | high | xhigh` it actually exposes (always at least `off`). The catalog (`GET /api/models`) surfaces each model's supported `efforts` through the gateway seam (pi-ai stays imported only in its adapter, ADR-0005), and the composer's effort dropdown shows only the selected model's levels — never a flat global list. When the user switches to a model that doesn't support a stored effort, the UI drops the incompatible level rather than sending it.
