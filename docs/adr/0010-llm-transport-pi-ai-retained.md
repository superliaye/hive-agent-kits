...
The bet is deliberately on the **stable `effect` core**, not on alpha `@effect/ai`. The two are separable: the typed-error channel and `Layer`/`Context` DI we depend on come from `effect`, not from the AI package.
...
- **Maturity.** `effect` core is stable; `@effect/ai` is alpha. Betting the transport on an alpha package couples a load-bearing seam to churn we don't control.
