...
1. **Audit** (`src/audit/audit.ts`) attaches a synchronous listener per audited event type; a normalizer throw or a `bun:sqlite` persist failure propagates up through `TypedEmitter.emit` and **fails the originating operation** (`audit.ts:58-64`). This is exactly the ADR-0004 invariant *"Failure semantics: block, with transaction split"* — audit-first ordering, no silent gaps.
...
**(b) How is block-on-failure / audit-first ordering preserved?** By keeping `TypedEmitter`'s sequential, throw-fails-the-emit dispatch (`typed-emitter.ts:24-31`) verbatim. Audit's listener stays synchronous and lets persist failures propagate (`audit.ts:58-64`), so an audit-write failure still fails the originating operation *before* it commits its side effect — the ADR-0004 invariant. The SSE relay's per-listener `try/catch` (`routes.ts:491-502`) continues to opt *out* of block-on-failure for dead clients. A standard `PubSub` does not give this for free; keeping `TypedEmitter` keeps it by construction.
