...
**Known gap (pre-existing code defect, tracked — not blessed by this ADR).** Block-on-failure is only realized when the *emit site* `await`s. Today only **Config** (`src/config/store.ts:53,88`) and **Catalog** `await events.emit(...)`. The audited **Secrets** (`src/secrets/store.ts:102,111,129,136`) and **Runs** (`src/runs/executor.ts:110,190,237,264`) emits use `void events.emit(...)` (fire-and-forget) while wired to Audit (`src/audit/subscriptions.ts:122-128`), so an audit-persist failure on `secret.write` / `run.started` becomes an unhandled rejection and does **not** fail the originating operation — and `secrets set()` commits `map.set`+`flush` *before* the discarded emit, so audit-first ordering is already broken there. Both `void`-emit patterns predate this branch; ADR-0004's uniform "no silent gaps" guarantee stands as the design intent, and these sites are a **defect to fix**, not behavior this ADR documents as acceptable. Remediation is tracked for Phase-4 §4.2: `await` the audited Secrets/Runs emits to restore block-on-failure + audit-first ordering, and add a project-wide, **non-suppressible** no-floating-promises lint (Biome 2.x `noFloatingPromises` — 1.9.4 does not ship it — configured to forbid `void`-ed promises too; inline suppression of the rule prohibited) so a `void`-ed audited emit cannot recur.

**A third site — a conscious exemption, not the same defect.** The *external-edit* config reload path (`src/config/store.ts:135`, `source:"external"`) also fire-and-forgets its audited `config.change` emit, but block-on-failure has no teeth here: the reload merely *observes* an already-committed external (e.g. vim) edit from a synchronous `fs.watch` callback that cannot `await`, and there is no originating operation to fail. This path stays **best-effort with a loud trace-log** — §4.2 de-floats it with an explicit `.catch(err => log().error(...))` (handled, *not* suppressed) so the lint passes while the reload never blocks. ADR-0004's *no silent gaps* narrows here to *no silent gaps; external-observation audit failures are loud-logged best-effort*.
...
- **Known block-on-failure gap carried forward, not closed here.** Audited Secrets/Runs emits are `void`-ed today, so they do not realize the guarantee this ADR keeps available (see *Known gap* in Context). Fixing the call sites and enforcing it with a non-suppressible no-floating-promises lint is tracked for Phase-4 §4.2; this ADR keeps the seam *capable* of the guarantee, the §4.2 code makes every audited site *claim* it.
