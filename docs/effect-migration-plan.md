...
- **4.2-B** — a project-wide, **non-suppressible** no-floating-promises guard so a `void`-ed audited emit cannot recur, split across two mechanisms because neither alone covers the directive:
  ...
  - Both are **scoped to `packages/daemon/src/` + `scripts/`** (`biome.json` `files.includes`; the script walks both roots, gating the gate dir itself) so the type scanner stays off the `packages/shell/`/`packages/ui/`/`packages/daemon/bundled/` broken-import corpus. A repo-wide `.gitattributes` (`* text=auto eol=lf`) pins every text file to LF so the AST scanner sees stable line endings on Windows checkouts. A tracked `.githooks/pre-commit` (wired via the package.json `prepare` script) runs `check:no-float` + `biome check` on every commit.
...
### 4.3 — Delete the proxies (done)

Production is fully migrated: `createServer()` resolves every migrated service (`Config`/`Secrets`/`Catalog`/`Audit` + `HiveDb`) off the single root `ManagedRuntime` (§4.1b); no production code calls a `createX()` proxy. Every proxy's only remaining consumers were the modules' plain-async **legacy-surface test suites**, now migrated to the `XLive` layers.
...
- **`createSecrets()` / `createConfig()` / `createAudit()` — DELETED — test suites migrated to the `XLive` layers.** Each suite now builds the service via `ManagedRuntime.make(XLive(opts))` + `runSync(X)` with per-test disposal (`secrets/__tests__/index.test.ts`; `config/__tests__/{store,persistence}.test.ts` via the generic `configRuntime()` accessor, since they use a TEST schema not `AppConfig`; `audit/__tests__/{audit,subscriptions}.test.ts`). Each `src/<m>/index.ts` is now a pure re-export barrel keeping the legacy surface type (`Secrets`/`Config<S>`/`Audit`), `CreateXOptions`, schema, and error re-exports that production still needs. `audit/audit.ts` was deleted outright (the `Audit` type is now an alias of the service value `AuditSvc`); `AuditSources.secrets` was narrowed to the consumer-owned `{ events }` port `wireSubscriptions` actually reads.

**End-state:** the literal "no proxy remains" is reached — no `createX()` proxy anywhere in `src/`, production or test; `src/` has no plain-async outside the I/O-edge adapters. **Do not migrate** the trace logger — `src/lib/log.ts` stays a call-site singleton by existing design (AGENTS.md). `ThreadsLive` (§4.x) was built for uniformity — the last module brought onto the root runtime.
