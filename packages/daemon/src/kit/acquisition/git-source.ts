...
export async function acquireGitSource(
  ...
  return locked(key, async () => {
    ...
    const run = async (args: readonly string[], phase: "fetch" | "archive" | "other" = "other") => {
      try {
        return await git.run(args, { env: daemonEnv, timeoutMs: limits.timeoutMs });
      } catch (error) {
    ...
    archive = (await run(["-C", cache, "archive", "--format=tar", treeish], "archive")).stdout;
    ...
    const stage = mkdtempSync(join(options.tmpRoot, "extract-git-"));
    try {
      extractBoundedTree(archive, stage, limits);
      ...
      throw new GitAcquireError("io", "selected tree could not be materialized");
    }
  });
}
