...
**Working Directory**:
The filesystem location a **Run** operates from — the `cwd` the **Agent Backend**'s vendor SDK is spawned in. Resolved per Run: a per-conversation choice, else the Agent's default, else a per-Agent workspace under `~/.hive`. Carries two patterns: an **agent bound to a repo** (set the Agent default to the repo root — a CLI started there inherits that repo's *own* committed Skills/MCP/conventions automatically, composing with the Agent's projected portable Skills) and an **agent you aim per conversation** (no Agent default; each Thread points at a repo, or a parent dir for cross-repo work). Distinct from **Domain**: Domain is the abstract focus label; Working Directory is the concrete operating location.
...
A **Thread** is always in one of three lifecycle states: _active_ (in the working list), _archived_ (set aside but fully readable), or _deleted_ (gone). There is no context-reset: Hive never trims, summarizes-down, or resets a conversation. A conversation is only ever archived or deleted — never truncated. Continuity within an **Agent Backend** is the backend's own native session resume: while a Thread stays on one backend, each Run picks up that backend's full prior history. On a cross-backend switch, the first **Run** sent on the new backend starts that backend's session fresh — prior history is not replayed into it (the conversation record itself is still kept whole).
...
**Run**:
One execution of an **Agent** on a **Thread**. Spans from invocation to completion — may include multiple model turns and tool calls. Multiple Runs of the same Agent (on the same or different Threads) may execute concurrently. A Run drives the Agent's **Agent Backend** — a vendor Agent SDK (`claude-code`/`codex`) that the SDK spawns locally and whose stream Hive folds into `RunEvent`s. From the outside (UI, CLI, audit), every Run is a stream of `RunEvent`s — the backend is invisible at the seam. Modeled after OpenAI's Run.
...
**Provider Hint**:
Typed escape-hatch metadata on a **Capability Manifest** carrying per-provider concessions (e.g., Anthropic `cache_control` placement, OpenAI strict-mode requirement, Gemini schema subset). Read at Run start to produce a provider-shaped artifact.
...
**Tool-use Loop**:
The iterative cycle inside a **Run**: the model emits a tool call → the call runs → the result returns to the model → the model continues. The vendor Agent SDK owns this loop; Hive observes it from the SDK's event stream and exposes its own domain capabilities to the loop over the MCP capability server.

**Audit Log**:
Append-only record of **what users and agents did** — tool calls observed from a backend's stream, Secrets access (the reference and source, never the value), MCP server lifecycle, Agent lifecycle (created/updated/destroyed by Agent Manager), backend run starts (backend kind + resolved model), Run lifecycle (started/completed/failed/cancelled), user actions (messages sent, model overrides, Harness binding edits). Primary purpose: answering "what happened, who triggered it, when?" Audit is now observation-without-enforcement — the SDK runs under bypass governance, so there is no permission decision to record (governance is deferred — ADR-0019). Schema reserves fields for v1.1 tamper-evidence (hash chain, signature) without requiring data migration. Replay-as-executable-history is an explicit non-goal for v1. **System-internal diagnostics (parse errors, watcher events, startup chatter, performance) live in the Trace Log — not here.** See [ADR-0004 "Audit vs trace"](docs/adr/0004-audit-log-design.md).

**Built on a subscribe model, not a push API.** Other modules do **not** call `audit.record(...)`. Each module owns its typed event stream (Run emits `RunEvent` + the backend run/tool-observed events, Secrets emits `SecretAccess`, MCP emits server-lifecycle events, the Capability Registry emits registration events, the Agent Manager emits lifecycle events). The Audit Log is a subscriber: it consumes these streams, normalizes each into a common `AuditEvent` row, and persists. Nothing reaches *into* Audit; Audit reaches *out*. This is the same pattern that lets the UI, future observability exporters, and any future training-data dumper plug in without touching emitters.

**Trace Log**:
Pino-backed JSONL diagnostic stream at `~/.hive/logs/daemon.log`. Captures the system's view of its own operation: malformed-manifest skip, filesystem watcher errors, daemon startup phases, hot-reload rescans, gateway latency, CLI subprocess output capture, anything you'd want to see in a stack trace. Modules import a singleton (`packages/daemon/src/lib/log.ts`) and write structured records directly — *no* subscribe pattern, because diagnostic context is best emitted at the call site with full local state. Trace answers "why didn't this work?"; Audit answers "what did the user/agent do?". The two stores are intentionally separate — different schemas, different retention, different consumers.
...
**Agent Backend**:
The vendor Agent SDK that executes an Agent's **Run**. The **Agent Harness** carries an authored default; like model and effort it is user-overridable per conversation (and promotable to the Agent default), so an Agent can run on one backend in a Thread and another elsewhere. Two backends in v1 — both drive a vendor SDK locally on the user's machine (ADR-0019):
- **`claude-code`** — drives `@anthropic-ai/claude-agent-sdk` (the SDK spawns its own bundled Claude Code binary).
- **`codex`** — drives `@openai/codex-sdk` (the SDK shells to its bundled `@openai/codex` binary).

For each, Hive resolves model + effort + auth, hands the SDK the task (Thread history + the user's latest message + the Agent's bound **Skills** *projected* into the SDK's native skill format + the authored prompt as the SDK's instructions), and folds the SDK's stream into `RunEvent`s. The SDK runs its **own** tool loop and progressive disclosure; Hive runs none. Hive's own domain capabilities (Memory, capability invocation, Agent-Manager lifecycle) are exposed as **one MCP server** both backends connect to by URL — the single "author tools once" boundary; file/shell tools are the SDK's own.
...
- Every **Run** drives a vendor Agent SDK (`claude-code`/`codex`) locally on the user's machine; the SDK manages its own LLM calls and tool loop, and Hive folds its stream into `RunEvent`s
- An **Agent Backend** is selected per-Agent on the **Agent Harness**; the Run module's interface (`startRun → AsyncIterable<RunEvent>`) is the seam where the two SDK backends are interchangeable
