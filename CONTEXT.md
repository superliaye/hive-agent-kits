...
**Agent Backend**:
The runtime that executes an Agent's **Run**. The **Agent Harness** carries an authored default; like model and effort it is user-overridable per conversation (and promotable to the Agent default), so an Agent can run `native` in one Thread and CLI-driven in another. Two kinds in v1:
...
The seam is the Run module's interface: `startRun(thread, agent) → AsyncIterable<RunEvent>`. Native and CLI-driven backends are interchangeable behind it — UI, CLI, audit, and Thread persistence see the same event shape. Every Agent may switch backend EXCEPT the **Agent Manager**, which is always `native` — its agent-lifecycle Tools (`create_agent`, `update_agent_harness`, `destroy_agent`) are in-process built-ins a CLI cannot invoke. The **Root Agent** may run CLI-driven; its `spawn_sub_agent` dispatch is simply unavailable there (the CLI does its own internal sub-tasking, opaque to Hive — see ADR-0018).
